...
define(["require", "exports", "../DreamSpace", "./Events", "./Diagnostics", "../Logging"], function (require, exports, DreamSpace_1, Events_1, Diagnostics_1, Logging_1) {
    ...
    (function (Browser) {
        ...
        (function (DOM) {
            ...
            DOM.onDOMLoaded = Events_1.EventDispatcher.new(DOM, "onDOMLoaded", true);
            ...
            DOM.onPageLoaded = Events_1.EventDispatcher.new(DOM, "onPageLoaded", true);
            /** Called when the page has loaded.
              * Returns true if running upon return, or already running, and false if not ready and queued to run later. */
            function onReady() {
                var log = Diagnostics_1.Diagnostics.log("DOM Loading", "Page loading completed; DOM is ready.").beginCapture();
                //??if ($ICE != null) {
                ...
                log.write("Dispatching DOM 'onReady event ...", Logging_1.LogTypes.Info);
                Browser.onReady.autoTrigger = true;
                Browser.onReady.dispatchEvent();
                log.write("'DreamSpace.DOM.Loader' completed.", Logging_1.LogTypes.Success);
                log.endCapture();
                return true;
            }
            ;
            ...
            function _doReady() {
                var log = Diagnostics_1.Diagnostics.log("DOM Loading", "Checking if ready...").beginCapture();
            ...
            function _doOnDOMLoaded() {
                if (!_domLoaded) {
                    _domLoaded = true;
                    var log = Diagnostics_1.Diagnostics.log("DOM Loading", "HTML document was loaded and parsed. Loading any sub-resources next (CSS, JS, etc.)...", Logging_1.LogTypes.Success).beginCapture();
            ...
            function _doOnPageLoaded() {
                if (!_pageLoaded) {
                    ...
                    _pageLoaded = true;
                    var log = Diagnostics_1.Diagnostics.log("DOM Loading", "The document and all sub-resources have finished loading.", Logging_1.LogTypes.Success).beginCapture();
