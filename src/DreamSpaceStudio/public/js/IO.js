...
class ResourceRequest extends Types_1.FactoryBase() {
    ...
    static set cacheBustingVar(value) { this._cacheBustingVar = Utilities_1.toString(value) || '_v_'; }
    ;
}
/**
    ...
    class $__type {
        ...
        get url() {
            if (typeof this._url == 'string' && this._url.charAt(0) == "~") {
                var _baseURL = Core_1.basePathFromResourceType(this.type);
        ...
        _Start(_method, _body, _username, _password) {
            ...
            if (this.status == Resources_1.RequestStatuses.Pending) {
                ...
                if (!DreamSpace.isDebugging() && typeof Storage !== void 0)
                    try {
                        var currentAppVersion = DreamSpace.getAppVersion();
                        var versionInLocalStorage = localStorage.getItem("version");
                        var appVersionInLocalStorage = localStorage.getItem("appVersion");
                        if (versionInLocalStorage && appVersionInLocalStorage && DreamSpace.version == versionInLocalStorage && currentAppVersion == appVersionInLocalStorage) {
                    ...
                    var loaded = () => {
                        if (xhr.status == 200 || xhr.status == 304) {
                            ...
                            if (this.type && responseType && this.type != responseType) {
                                ...
                                if (!DreamSpace.isDebugging() && typeof Storage !== void 0)
                                    try {
                                        localStorage.setItem("version", DreamSpace.version);
                                        localStorage.setItem("appVersion", DreamSpace.getAppVersion());
            ...
            try {
                ...
                if (typeof payload == 'object' && payload.__proto__ == PrimitiveTypes_1.CoreObject.prototype) {
            ...
            if (this.status == Resources_1.RequestStatuses.Ready) {
                if (this._onReady && this._onReady.length) {
                    try {
                        ...
                        if (DreamSpace.isDebugging() && (this.type == Resources_1.ResourceTypes.Application_Script || this.type == Resources_1.ResourceTypes.Application_ECMAScript))
        ...
        _doError() {
            ...
            if (this.status == Resources_1.RequestStatuses.Error) {
                ...
                if (msgs)
                    ...
                    msgs = ".";
                throw new Error("Unhandled error loading resource " + (typeof this.type == 'number' ? Resources_1.ResourceTypes[this.type] : this.type) + " from '" + this.url + "'" + msgs + "\r\n");
            }
        }
...
})(ResourceRequest = exports.ResourceRequest || (exports.ResourceRequest = {}));
// ===============================================================================================================================
