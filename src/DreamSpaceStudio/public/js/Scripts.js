...
(function (Scripts) {
    ...
    // ====================================================================================================================
    class ScriptResource extends FactoryBase(System.IO.ResourceRequest) {
    }
    Scripts.ScriptResource = ScriptResource;
    (function (ScriptResource) {
        class $__type extends FactoryType(System.IO.ResourceRequest) {
    ...
    */
    class Manifest extends FactoryBase(ScriptResource) {
    }
    ...
    function getManifest(path) {
        ...
        request = (Manifest.new(path)).then((request, data) => {
            ...
            if (script) {
                var matches = DreamSpace.matches(Scripts.MANIFEST_DEPENDENCIES_REGEX, script); // ("a.b.x, a.b.y, a.b.z")
                if (matches.length) {
                    var dependencies = [];
    ...
    class Module extends FactoryBase(ScriptResource) {
    }
    ...
    (function (Module) {
        ...
        $__type._globalaccessors = (() => { return safeEval("({ get: function(varName) { return p0.global[varName]; }, set: function(varName, val) { return p0.global[varName] = val; } })", DreamSpace); })();
    ...
    * Note: This should either be empty, or always end with a URL path separator ('/') character (but the system will assume to add one anyhow if missing). */
    Scripts.pluginFilesBasePath = System.IO && System.IO.Path ? System.IO.Path.combine(baseURL, "wwwroot/js/") : baseURL + "wwwroot/js/";
    ...
    function module(dependencies, moduleFullTypeName, moduleFileBasePath = null, requiresGlobalScope = false) {
        ...
        if (path && path.charAt(0) == '~')
            path = System.IO.Path.combine(Scripts.pluginFilesBasePath, path.substring(1)); // ('~' is a request to insert the current default path; eg. "~DreamSpace.System.js" for "DreamSpaceJS/DreamSpace.System.js")
