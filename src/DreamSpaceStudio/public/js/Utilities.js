...
(function (Utilities) {
    ...
    function toString(value) {
        if (arguments.length == 0)
            return $__ROOT_DREAMSPACE_NAMESPACE_NAME;
    ...
    */
    function dereferencePropertyPath(path, origin, unsafe = false) {
        if (unsafe)
            return DreamSpace.safeEval('p0.' + path, origin); // (note: this is 'DreamSpace.eval()', not a direct call to the global 'eval()')
        if (origin === void 0 || origin === null)
            origin = this !== DreamSpace.global ? this : DreamSpace.global;
