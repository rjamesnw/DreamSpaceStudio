 ...
 */
exports.viewsRoot = DS.Path.combine(DS.webRoot, "features");
function createCallableViewAttributes() {
    var attributes = {};
    var attributesObject = function (p1, p2, p3) {
        var s = "", value;
        if (typeof p1 == 'string') {
            let attrName = p1;
            if (!attrName)
                throw new DS.Exception("An attribute name is required.");
            value = attributes[attrName];
            let p2Defined = !util_1.isNullOrUndefined(p2);
            let p3Defined = !util_1.isNullOrUndefined(p3);
            if (p2Defined || p3Defined) {
                if (!util_1.isNullOrUndefined(p2))
                    s = DS.StringUtils.append(DS.nud(p2, ''), DS.nud(value, ''), ' ');
                else
                    s = '' + value;
                if (!util_1.isNullOrUndefined(p3))
            ...
            let attributesToIgnore = p1;
            let attributesToInclude = p2;
            for (var p in attributes) {
                if (attributesToIgnore && attributesToIgnore.indexOf(p) >= 0)
                    continue;
                if (attributesToInclude && attributesToInclude.indexOf(p) < 0)
                    continue;
                value = attributes[p];
...
class HttpContext {
    constructor(requestOrCtx, responseOrVewData, viewDataOrBasePath, viewBasePath) {
...
function _processExpression(httpContext, expr, dataContext) {
    try {
        var compiledExpression = expr.replace(/({){|(})}/g, "$1$2").replace(/\[@]\./g, "$['@'].attributes.").replace(/\[@]\(/g, "$['@'](")
            .replace(/(\[\[[^\]]*]]|'(?:\\'|[^'])*'|"(?:\\"|[^"])*")|(^|[^a-z0-9_$.)[\]'"`\/\\])(\.|\[(?!\[))/gmi, function (match, p1, p2, p3, offset, src) {
 ...
 */
exports.__express = function (filePath, httpContext, callback) {
    try {
        ...
        fs.readFile(filePath, function (err, fileContent) {
            ...
            try {
                ...
                for (var i = 0, n = textParts.length; i < n; ++i) {
                    ...
                    if (isTokenExpression || isStartServerTagToken || isEndServerTagToken) {
                        ...
                        if (isCommand || isServerTag) {
                            ...
                            if (isServerTag)
                                _cmd = '#' + cmd.substr(1); // (keep things simple and just convert this into a proper command)
                            try {
                                switch (_cmd) {
                                    case "#":
                                        expr = argStr;
                                        ...
                                        value = () => __awaiter(this, void 0, void 0, function* () {
                                            ...
                                            if (!_viewScope.httpContext.sectionManager)
                                                if (optional)
                                                    ...
                                                    throw new DS.Exception(`There are no sections defined.`);
                                            if (sectionNameToRender == SectionManager.defaultSectionName) {
                                                ...
                                                value = yield _viewScope.childContent.render();
                                                ...
                                                if (!sectionManager.hasSection(sectionNameToRender))
                                                    if (optional)
                                                        ...
                                                        throw new DS.Exception(`There is no section defined with the name '${sectionNameToRender}'. You can append the '?' character to the name if the section is optional.`);
                                                var section = _viewScope.httpContext.sectionManager.sections[sectionNameToRender];
                                                value = yield section.render();
                                            }
                                            return value;
                                        });
                                        if (sectionNameToRender == SectionManager.defaultSectionName)
                                    ...
                                    case "#view": {
                                        ...
                                        if (selfClosing)
                                            argStr = argStr.substr(0, argStr.length - 1).trimRight();
                                        let viewPath, viewHttpContext = new HttpContext(httpContext, include ? void 0 : new ViewData());
                                        if (!include)
                                        ...
                                        if (isServerTag) {
                                            ...
                                            for (let i = 0, n = attributes.length; i < n; ++i) {
                                                ...
                                                if (equalIndex >= 0) {
                                                    ...
                                                    name = attribute.trim(); // (allow setting names without values; i.e. 'readonly')
                                                ...
                                                if (name[0] == '$') {
                                                    ...
                                                    if (name.substr(0, 6) == '$data-') {
                                                        ...
                                                        switch (name) {
                                                            ...
                                                            case '$':
                                                            case '$expression':
                                                                _processExpression(viewHttpContext, value, viewHttpContext.viewData);
                                                                break; // (this runs in the nested view context)
                ...
                let _i1 = i >= coderadius ? i - coderadius : 0, _i2 = i + coderadius < textParts.length ? i + coderadius : textParts.length - 1, s, htmlsegmentL = "", tokenSegment = "", htmlsegmentR = "";
...
function apply(app, viewsRootPath = exports.viewsRoot) {
    // ... view engine setup ...
    app.engine('t.html', exports.__express);
    exports.viewsRoot = DS.Path.isAbsolute(viewsRootPath) ? viewsRootPath : DS.Path.combine(DS.webRoot, viewsRootPath);
    app.set('views', exports.viewsRoot); // specify the views directory
    app.set('view engine', 't.html'); // register the template engine
}
exports.apply = apply;
//# sourceMappingURL=t.html.js.map