...
(() => {
    ...
    if (":".split(/:/g).length == 0) {
        (<any>String.prototype)['$__DreamSpace_oldsplit'] = String.prototype.split; // (this is only executed once because of the ext line)
        String.prototype.split = function (separator: any, limit?: number, delimiterList?: string[]): string[] {
            ...
            if (delimiterList)
                ...
                return (<any>String.prototype)['$__DreamSpace_oldsplit'](separator, limit); // (old function works find for non-RegExp splits)
            else
