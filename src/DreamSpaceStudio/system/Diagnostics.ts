...
namespace DS {
    var base_log = log; // (need a reference to the basic root level log function)
    ...
    export namespace Diagnostics {
        ...
        export class LogItem {
            ...
            constructor(parent: ILogItem, title: any, message: any, type: LogTypes = LogTypes.Normal, outputToConsole = true) {
                if (title === void 0 || title === null) {
                    if (DS.isEmpty(message))
                        error("LogItem()", "A message is required if no title is given.", this);
                    title = "";
                }
                ...
                if (console && outputToConsole) { // (if the console object is supported, and the user allows it for this item, then send this log message to it now)
                    ...
                    if (TimeSpan) {
                        var time = TimeSpan.utcTimeToLocalTime(this.time);
                    ...
                    else consoleText = (new Date()).toLocaleTimeString() + " " + margin + _title + this.message; // TODO: Make a utility function to format Date() similar to hh:mm:ss
        ...
        export function getLogAsHTML(): string {
            ...
            for (i = 0, n = orderedLogItems.length; i < n; ++i) {
                ...
                if (cssAndIcon.cssClass)
                    messageHTML = cssAndIcon.icon + "<strong>" + StringUtils.replace(logItem.message, "\r\n", "<br />\r\n") + "</strong>";
                else
