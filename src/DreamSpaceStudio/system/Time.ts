...
namespace DS {
    ...
    export class TimeSpan {
        ...
        static msFromTime(year: number = DS.Time.__EpochYear, dayOfYear: number = 1, hours: number = 0, minutes: number = 0, seconds: number = 0, milliseconds: number = 0) {
        ...
        /** Returns the time zone as a string in the format "UTC[+/-]####".
            ...
            */
        static getTimeZoneSuffix(timezoneOffsetInMs: number = DS.Time.__localTimeZoneOffset): string {
            var tzInHours = -(timezoneOffsetInMs / DS.Time.__millisecondsPerHour);
            var hours = Math.abs(tzInHours);
            return "UTC" + (tzInHours >= 0 ? "+" : "-")
                + StringUtils.pad(Math.floor(hours), 2, '0')
                + StringUtils.pad(Math.floor(hours % 1 * DS.Time.__minsPerHour), 2, '0');
        }

        /** Returns the ISO-8601 time zone as a string in the format "[+/-]hh:mm:ss.sssZ".
            * @param {number} timezoneOffsetInMs The number of milliseconds to offset from local time to UTC time (eg. UTC-05:00 would be -(-5*60*60*1000), or 18000000).
            */
        static getISOTimeZoneSuffix(timezoneOffsetInMs: number = DS.Time.__localTimeZoneOffset): string {
            var tzInHours = -(timezoneOffsetInMs / DS.Time.__millisecondsPerHour);
            var hours = Math.abs(tzInHours);
            var minutes = Math.abs(hours % 1 * DS.Time.__minsPerHour);
            var seconds = minutes % 1 * DS.Time.__secondsPerMinute;
            return (tzInHours >= 0 ? "+" : "-")
                + StringUtils.pad(hours, 2, '0') + ":"
                + StringUtils.pad(minutes, 2, '0') + ":"
                + StringUtils.pad(Math.floor(seconds), 2, '0') + "."
                + StringUtils.pad(Math.floor(seconds % 1 * 1000), 3, null, '0') // (1000th decimal precision)
                + "Z";
        }
        ...
        toString(includeTime: boolean = true, includeMilliseconds: boolean = true, includeTimezone: boolean = true): string {
            ...
            return "Year " + StringUtils.pad(localTS.year, 4, '0') + ", Day " + StringUtils.pad(localTS.dayOfYear, 3, '0')
                + (includeTime ? " " + StringUtils.pad(localTS.hours, 2, '0') + ":" + StringUtils.pad(localTS.minutes, 2, '0') + ":" + StringUtils.pad(localTS.seconds, 2, '0')
        ...
        toUTCString(includeTime: boolean = true, includeMilliseconds: boolean = true): string {
            return "Year " + StringUtils.pad(this.year, 4, '0') + ", Day " + StringUtils.pad(this.dayOfYear, 3, '0')
                + (includeTime ? " " + StringUtils.pad(this.hours, 2, '0') + ":" + StringUtils.pad(this.minutes, 2, '0') + ":" + StringUtils.pad(this.seconds, 2, '0')
        ...
        toISODateString(includeTime: boolean = true, includeMilliseconds: boolean = true, includeTimezone: boolean = true): string {
            ...
            return StringUtils.pad(this.__date.getFullYear(), 4, '0') + "-" + StringUtils.pad(1 + this.__date.getMonth(), 2, '0') + "-" + StringUtils.pad(this.__date.getDate(), 2, '0')
                + (includeTime ? "T" + StringUtils.pad(this.__date.getHours(), 2, '0') + ":" + StringUtils.pad(this.__date.getMinutes(), 2, '0') + ":" + StringUtils.pad(this.__date.getSeconds(), 2, '0')
        ...
        toUTCISODateString(includeTime: boolean = true, includeMilliseconds: boolean = true, includeTimezone: boolean = true): string {
            if (!this.__date)
                this.__date = new Date(this.toValue());
            return StringUtils.pad(this.year, 4, '0') + "-" + StringUtils.pad(1 + this.__date.getUTCMonth(), 2, '0') + "-" + StringUtils.pad(this.__date.getUTCDate(), 2, '0')
                + (includeTime ? "T" + StringUtils.pad(this.hours, 2, '0') + ":" + StringUtils.pad(this.minutes, 2, '0') + ":" + StringUtils.pad(this.seconds, 2, '0')
                    + (includeMilliseconds && this.milliseconds ? "." + this.milliseconds : "")
