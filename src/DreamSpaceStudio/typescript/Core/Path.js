...
(function (Path) {
    ...
    function parse(url) {
        if (typeof url != 'string')
            url = Utilities_1.Utilities.toString(url);
    ...
    function combine(path1, path2, normalizePathSeparators = false) {
        if (typeof path1 != 'string')
            path1 = Utilities_1.Utilities.toString(path1);
        if (typeof path2 != 'string')
            path2 = Utilities_1.Utilities.toString(path2);
        if (path2.charAt(0) == '~')
    ...
    function resolve(path, currentResourceURL = Globals_1.DreamSpace.global.location.href, baseURL = Globals_1.DreamSpace.baseURL) {
        ...
        path = Utilities_1.Utilities.toString(path).trim();
        if (!path)
    ...
    function setLocation(url, includeExistingQuery = false, bustCache = false) {
        var query = Query_1.Query.new(url);
        if (bustCache)
            query.values[ResourceRequest_1.ResourceRequest.cacheBustingVar] = Date.now().toString();
        if (includeExistingQuery)
            query.addOrUpdate(Query_1.pageQuery.values);
        if (url.charAt(0) == '/')
            url = resolve(url);
