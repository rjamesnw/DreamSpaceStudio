...
export abstract class Module extends Factory(ScriptResource) implements IModuleLoadEvents {
    ...
    private $__modFunc: (ds: typeof DS, module: IModule, exports: {}, ...args: any[]) => _IModuleAccessors;
    ...
    private static readonly _globalaccessors: _IModuleAccessors = (() => { return DS.safeEval("({ get: function(varName) { return p0.global[varName]; }, set: function(varName, val) { return p0.global[varName] = val; } })", DS); })();
    ...
    execute(useGlobalScope = false): void {
        if (this.status == RequestStatuses.Ready && !this._moduleGlobalAccessors) {
            ...
            if (useGlobalScope) {
                ...
                this._moduleGlobalAccessors = this.$__modFunc(DS, this, this.exports, tsHelpers); // (note that 'this.' effectively prevents polluting the global scope in case 'this' is used)
            }
