...
export class ApplicationElement extends Factory(HTMLElement) {
    static 'new'(title: string, description: string, targetElement: NativeTypes.IHTMLElement = null): ApplicationElement { return null; }

    static init(o: ApplicationElement, isnew: boolean, title: string, description: string, targetElement: NativeTypes.IHTMLElement = null) {
    ...
    */
    ...
    updateLayout(recursive: boolean = true) {
        var log = DS.isDebugging() ? Diagnostics.log(ApplicationElement, "Application is updating its layout ...").beginCapture() : null;

        super.updateLayout(recursive);

        if (this.__element != null) {
            var node = <NativeTypes.IHTMLElement & IndexedObject>this._targetElement.firstChild;
            ...
            if (htmlFileURI) {
                ...
                request.onreadystatechange = () => {
                    if (request.readyState == (XMLHttpRequest.DONE || 4)) {
                        var ok = false, response = request.response || request.responseText || request.responseXML || (<IndexedObject>request)['responseBody'];
