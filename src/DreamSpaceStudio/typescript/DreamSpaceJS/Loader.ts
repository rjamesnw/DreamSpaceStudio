...
export namespace Loader {
    ...
    export var rootBootPath: string = rootBootPath && ('' + rootBootPath) || "~/DreamSpace/";
    ...
    export function bootstrap() {
        ...
        log("Loader.BootSubPath", rootBootPath + " (if this path is wrong set 'DreamSpace.Loader.BootSubPath' before 'Loader.bootstrap()' gets called)");
        ...
        get(rootBootPath + "DreamSpace.Polyfills.js").ready(onReady) // (some polyfills may be needed by the system)
            .include(get(rootBootPath + "DreamSpace.Types.js")).ready(onReady) // (common base types)
            .include(get(rootBootPath + "DreamSpace.Utilities.js")).ready(onReady) // (a lot of items depend on time utilities [such as some utilities and logging] so this needs to be loaded first)
            .include(get(rootBootPath + "DreamSpace.Globals.js")).ready(onReady) // (a place to store global values [especially global scope callbacks required by some libraries, such as Google Maps, etc.] without polluting the global scope)
            .include(get(rootBootPath + "DreamSpace.Scripts.js")).ready(onReady) // (supports DreamSpace-based module loading)
            ...
            .include(get(rootBootPath + "System/DreamSpace.System.js").ready(onReady)) // (any general common system properties and setups)
            .include(get(rootBootPath + "System/DreamSpace.System.PrimitiveTypes.js").ready(onReady)) // (start the primitive object definitions required by more advanced types)
            .include(get(rootBootPath + "System/DreamSpace.System.Time.js")).ready(onReady) // (extends the time utilities and constants into a TimeSpan wrapper)
            .include(get(rootBootPath + "System/DreamSpace.System.Exception.js").ready(onReady)) // (setup exception support)
            .include(get(rootBootPath + "System/DreamSpace.System.Diagnostics.js")).ready(onReady) // (setup diagnostic support)
            .include(get(rootBootPath + "System/DreamSpace.System.Events.js").ready(onReady)) // (advanced event handling)
            .include(get(rootBootPath + "System/DreamSpace.System.Browser.js")).ready(onReady) // (uses the event system)
            .include(get(rootBootPath + "System/DreamSpace.System.Text.js").ready(onReady)) // (utilities specific to working with texts)
            .include(get(rootBootPath + "System/DreamSpace.System.Data.js").ready(onReady))
            .include(get(rootBootPath + "System/DreamSpace.System.IO.js").ready(onReady)) // (adds URL query handling and navigation [requires 'Events.EventDispatcher'])
            .include(get(rootBootPath + "System/DreamSpace.System.Storage.js").ready(onReady)) // (utilities for local storage support in DreamSpace)
            .ready(() => {
        ...
        DreamSpace.Loader.bootstrap = <any>noop(); // (prevent this function from being called again)
    }

    // ========================================================================================================================================

    /** This is provided to allow callbacks to run just before 'DreamSpace.Loader.bootstrap()' gets called (depending on the 'DreamSpace.debugMode' setting). 
     * * Setting 'DreamSpace.debugMode' to 'DreamSpace.DebugModes.Debug_Wait' prevents 'DreamSpace.Loader.bootstrap()' from getting called automatically.
