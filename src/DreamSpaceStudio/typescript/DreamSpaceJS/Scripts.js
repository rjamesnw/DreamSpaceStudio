...
const PrimitiveTypes_1 = require("./System/PrimitiveTypes");
...
function moduleNamespaceToFolderPath(nsName) {
    ...
    var sysNs1 = Types_1.nameof(() => DreamSpace.Scripts.Modules) + "."; // (account for full names or relative names)
    var sysNs2 = Types_1.nameof(() => Scripts.Modules) + ".";
    var sysNs3 = Types_1.nameof(() => Modules) + ".";
...
// ====================================================================================================================
class ScriptResource extends Types_1.Factory(IO_1.default.ResourceRequest) {
...
*/
class Manifest extends Types_1.Factory(ScriptResource) {
}
exports.Manifest = Manifest;
(function (Manifest) {
    // (evaluated last, but called first)
    class $__type extends FactoryType(ScriptResource) {
        /** A convenient script resource method that simply Calls 'Globals.register()'. For help, see 'DreamSpace.Globals' and 'Globals.register()'. */
        registerGlobal(name, initialValue, asHostGlobal) {
            return Globals.register(this, name, initialValue, asHostGlobal);
        }
...
class ScriptError {
    ...
    get message() { return getErrorMessage(this); }
...
function getManifest(path) {
    ...
    request = (Manifest.new(path)).then((request, data) => {
        ...
        if (script) {
            var matches = DreamSpace.matches(exports.MANIFEST_DEPENDENCIES_REGEX, script); // ("a.b.x, a.b.y, a.b.z")
            if (matches.length) {
                var dependencies = [];
        ...
        var sourcePragmaInfo = Core_1.extractPragmas(script);
        ...
        try {
            ...
            errorResult = ScriptError.fromError(ex, func && func.toString() || script, Types_1.nameof(() => DreamSpace.Scripts.getManifest, true) + "() while executing " + manifestRequest.url);
...
(function (Module) {
    class $__type extends FactoryType(ScriptResource) {
        ...
        execute(useGlobalScope = false) {
            if (this.status == Resources_1.RequestStatuses.Ready && !this._moduleGlobalAccessors) {
                ...
                var accessors;
                if (useGlobalScope) {
                    this._moduleGlobalAccessors = (Globals_1.DreamSpace.globalEval(this.response), $__type._globalaccessors); // (use the global accessors, as the module was run in the global scope)
                }
                else {
        ...
        static [Globals_1.DreamSpace.constructor](factory) {
            factory.init = (o, isnew, fullname, url, minifiedUrl) => {
                factory.super.init(o, isnew, [Globals_1.DreamSpace.debugMode ? url : (minifiedUrl || url)]);
    ...
    $__type._globalaccessors = (() => { return Globals_1.DreamSpace.safeEval("({ get: function(varName) { return p0.global[varName]; }, set: function(varName, val) { return p0.global[varName] = val; } })", DreamSpace); })();
...
exports.pluginFilesBasePath = System.IO && System.IO.Path ? System.IO.Path.combine(baseURL, "wwwroot/js/") : baseURL + "wwwroot/js/";
...
function module(dependencies, moduleFullTypeName, moduleFileBasePath = null, requiresGlobalScope = false) {
    ...
    if (results[1]) {
        ...
        moduleFullTypeName = translateModuleTypeName(moduleFullTypeName); // (translate "System." to "DreamSpace." and "." to "DreamSpace." internally)
    ...
    if (path && path.charAt(0) == '~')
        path = System.IO.Path.combine(exports.pluginFilesBasePath, path.substring(1)); // ('~' is a request to insert the current default path; eg. "~DreamSpace.System.js" for "DreamSpaceJS/DreamSpace.System.js")
    ...
    var usingPluginFunc = ((onready, onerror) => {
        ...
        switch (mod.status) {
            case Resources_1.RequestStatuses.Error: throw DreamSpace.System.Exception.from("The module '" + mod.fullname + "' is in an error state and cannot be used.", mod);
