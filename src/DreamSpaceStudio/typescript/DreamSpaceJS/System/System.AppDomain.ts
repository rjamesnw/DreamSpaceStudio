...
import { DreamSpace as DS, IDreamSpace as IDS } from "./System";
import { IDisposable, extendNS, IFactory } from "../Types";
...
export interface IDomainObjectInfo extends IDisposable {
    ...
    /** 
      ...
      */
    $__appDomainId?: number; // (value is set and maintained by a Collections.IndexedObjectCollcetion instance; see 'DreamSpace.objects' [static collection])
     ...
     */
    $__globalId?: string; // (value is set and maintained by a Collections.IndexedObjectCollcetion instance; see 'DreamSpace.objects' [static collection])
...
// ... the following system namespace will extend the existing system namespace ...

eval(extendNS(() => System, () => DS.System));
        ...
        export class $__type extends FactoryType(CoreObject) {
            ...
            with<TFactory extends IFactory>(type: TFactory): TFactory {
                ...
                if (!typeInfo.$__parent || !typeInfo.$__name || !typeInfo.$__fullname)
                    throw Exception.error("with()", "The specified type has not yet been registered properly. Extend from 'DreamSpace.ClassFactory()' or use utility functions in 'DreamSpace.Types' when creating factory types.", type);
...
export interface IDreamSpace extends IDS { Windows: typeof Windows; }
(<IDreamSpace>DS).Windows = Windows;
