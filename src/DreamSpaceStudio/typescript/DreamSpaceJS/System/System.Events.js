...
// ############################################################################################################################
define(["require", "exports", "../Globals", "../Types", "./Exception", "./PrimitiveTypes"], function (require, exports, Globals_1, Types_1, Exception_1, PrimitiveTypes_1) {
    "use strict";
    ...
    var Events;
    (function (Events) {
        namespace(() => DreamSpace.System.Events);
        ;
        ;
        ...
        (function (EventDispatcher) {
            class $__type extends PrimitiveTypes_1.DependentObject {
                constructor() {
                    super(...arguments);
                ...
                attach(handler, eventMode = EventModes.Capture) {
                    if (this._getHandlerIndex(handler) == -1) {
                        var delegate = handler instanceof Delegate.$__type ? handler : Delegate.new(this, handler);
                ...
                __exception(msg, error) {
                    ...
                    return Exception_1.Exception.error("{EventDispatcher}.dispatchEvent():", "Error in event " + this.__eventName + " on object type '" + getTypeName(this.owner) + "': " + msg, { exception: error, event: this, handler: this.__handlerCallInProgress });
                }
    ...
    (function (Browser) {
        ...
        Browser.onReady = Events.EventDispatcher.new(Browser, "onReady", true);
    ...
    /** Triggered when all manifests have loaded. No modules have been executed at this point.
      ...
      */
    exports.onReady = Events.EventDispatcher.new(DreamSpace, "onReady", true);
});
// ############################################################################################################################
