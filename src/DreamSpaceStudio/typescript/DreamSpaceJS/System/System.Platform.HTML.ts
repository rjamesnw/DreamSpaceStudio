...
import { Window, IWindow } from "./Platform.Windows";
...
/** A context is a container that manages a reference to a global script environment. Each new context creates a new 
  ...
  */
@factory(this)
export class BrowserContext extends Context {
    ...
    _setupIFrame() {
        ...
        DS.global.document.body.appendChild(this._iframe);
            ...
            case DS.Environments.Browser:
                switch (contextType) {
                    ...
                    case Contexts.UnsecureWindow:
                        ...
                        break;
                    case Contexts.Local:
                        this._target = this._global = DS.global;
                        break;
                }
                break;
        }
...
@factory(this)
export class HTMLNode extends Factory(GraphNode) {
...
/** Represents an HTML node graph item that renders the content in the 'innerHTML of the default '__htmlTag' element (which is set to 'GraphItem.defaultHTMLTag' [DIV] initially).
  ...
  */
@factory(this)
export class HTMLElement<TElement extends NativeTypes.IHTMLElement = NativeTypes.IHTMLElement> extends Factory(HTMLNode) {
    ...
    static init<TName extends keyof HTMLElementTagNameMap = 'div', TElement extends HTMLElementTagNameMap[TName] = HTMLDivElement>
        (o: HTMLElement<TElement>, isnew: boolean, parent: IGraphNode, id?: string, name?: string, tagName: TName = <any>HTMLElement.defaultHTMLTagName, html?: string) {
    ...
    applyTo(el: NativeTypes.IHTMLElement, ignore?: string[]) {
        for (var p in this.__properties)
            if (!ignore || !ignore.indexOf || ignore.indexOf(p) == -1)
                try {
                    ...
                    if (prop)
                        if (p in el)
                            (<IndexedObject>el)[p] = prop.getValue();
                        else if (el.setAttribute)
                            el.setAttribute(p, prop.getValue());
                } catch (ex) { throw Exception.error("HTMLElement.applyTo()", `Error setting property/attribute '${p}' for element '${this.tagName}': ` + ex, this); }
    }
...
export interface IHTMLElement<TElement extends InstanceType<typeof DS.global.HTMLElement> = InstanceType<typeof DS.global.HTMLElement>>
    extends HTMLElement<TElement> { }
...
/** 
  ...
  */
@factory(this)
export class PlainText extends Factory(HTMLNode) {
    static 'new'(parent: IGraphNode, text: string = ""): IPlainText { return null; }

    static init(o: PlainText, isnew: boolean, parent: IGraphNode, text: string = "") {
        this.super.init(o, isnew, parent);
        o.text(text);
        o.tagName = "";
...
/** Represents an HTML text node graph item that renders the content in the 'innerHTML of a SPAN element. For plain text nodes use 'PlainText'.
  */
@factory(this)
export class HTMLText extends Factory(HTMLElement) {
...
@factory(this)
export class Phrase extends Factory(HTMLElement) {
    static 'new'(parent: IGraphNode, phraseTypeFlags: PhraseTypes = 0, html: string = ""): IPhrase { return null; }

    static init(o: Phrase, isnew: boolean, parent: IGraphNode, phraseTypeFlags: PhraseTypes = 0, html: string = "") {
...
/** Represents an HTML header element. 
  */
@factory(this)
export class Header extends Factory(HTMLElement) {
...
/** Represents an HTML body element. 
  */
@factory(this)
export class Body extends Factory(HTMLElement) {
    static 'new'(parent: IGraphNode, bodyLevel: number = 1, html: string = ""): IBody { return null; }

    static init(o: IBody, isnew: boolean, parent: IGraphNode, html = "") {
        this.super.init<any, any>(o, isnew, parent, html);
    }
                        ...
                        if (htmlReader.readMode == HTMLReaderModes.Attribute) {
                            ...
                            if (mode == 2 && htmlReader.readMode == HTMLReaderModes.Tag && htmlReader.isClosingTag()) { // (this an ending tag (i.e. "</...>"))
                                ...
                                if (graphItemType && classMatch.test(graphItemType)) {
                                    ...
                                    nodeType = <TNodeFactoryType>Utilities.dereferencePropertyPath(translateModuleTypeName(graphItemType), (<ITypeInfo>DreamSpace).$__parent);
