...
/** Since some plugins require global callbacks, such as when an API gets initialized at some time in the future (i.e. Google Maps, etc.),
...
namespace DreamSpace {
    ...
    */
    ...
    export function renderHelpers() {
        return "var __helpers = " + $__ROOT_DREAMSPACE_NAMESPACE_NAME + ".renderHelperVarDeclarations" + "('__helpers[1]'); eval(__helpers[0]);";
    }

}

eval("eval(" + $__ROOT_DREAMSPACE_NAMESPACE_NAME + ".renderHelpers())"); // (setup the global helpers to begin with)
