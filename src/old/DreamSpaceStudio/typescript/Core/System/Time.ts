...
export class TimeSpan extends Factory(Object) {
    ...
    static now(): ITimeSpan { return Date.now ? TimeSpan.new(Date.now()) : TimeSpan.fromDate(new Date()); }
    ...
    static utcTimeToLocalTime(timeInMs: number): ITimeSpan {
        return TimeSpan.new((timeInMs || 0) - DS.Time.__localTimeZoneOffset);
    }
    ...
    static getTimeZoneSuffix(timezoneOffsetInMs: number = DS.Time.__localTimeZoneOffset): string {
        ...
        return "UTC" + (tzInHours >= 0 ? "+" : "-")
            ...
            + String.pad(Math.floor(hours % 1 * DS.Time.__minsPerHour), 2, '0');
    }
