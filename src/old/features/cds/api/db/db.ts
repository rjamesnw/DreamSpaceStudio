...
export var wb_db = new DS.DB.MSSQL.MSSQLAdapter({
    user: process.env.WB_MSSQL_USER,
    password: process.env.WB_MSSQL_PASS,
    server: process.env.WB_MSSQL_HOST + (process.env.WB_MSSQL_INSTANCE ? '\\' + process.env.WB_MSSQL_INSTANCE : ''), // You can use 'localhost\\instance' to connect to named instance
...
export var wb_conn: DS.DB.MSSQL.MSSQLConnection;
...
export async function getWBConnection() {
    //if (this._BotPalConn && (this._BotPalConn.connection.state == 'connected' || this._BotPalConn.connection.state == 'authenticated'))
    //    return this._BotPalConn;
    return wb_conn || (wb_conn = await wb_db.createConnection()); // (uses the environment default values)
}
